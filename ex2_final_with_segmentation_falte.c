...
printf("\n**IN PIPE CMD**\n");
		...
		//----------------------------------------------------------------|
				...
				if(child_pid == 0)//if sons son									//*****PROBLAME IS HERE!!*****
				{
					...
					execvp(first_half_of_commandArray[0], first_half_of_commandArray);
						...
						close(pipe_fd[0]);
						// execute left cmd
						execvp(second_half_of_commandArray[1/*"0" and STDOUT_FILENO */], second_half_of_commandArray);
						}
				}//end of "if pipe"	
			}				
		}
//--------------------------------------------------------------------------------------------------------			
				else if(checkpipe(commandArray) == -1)	
				{
