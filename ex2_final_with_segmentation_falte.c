...
printf("\nfirst_half_of_commandArray[i1]: %s\n",first_half_of_commandArray[i1]);					
				}
...
printf("\nsecond_half_of_commandArray[i2]: %s\n",second_half_of_commandArray[i2]);			
				}
...
printf("\nDONE LEFT CMD\n");						
				...
				{printf("ERR\n");	exit(1);}		
					...
					{
		printf("\nENTERED TO RIGHT CMD\n");
						close(pipe_fd[1]);
						// replace standard output with output part of pipe
						dup2(pipe_fd[0], STDIN_FILENO);
printf("\nCHECK TO DE FACE!! RIGHT\n");						
						// execute right cmd
